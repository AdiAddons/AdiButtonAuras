...
In several handlers, possibly from from differents rules, apply to the same spell, they are called in order of definition. Latter handlers could see the results of previous ones in @model@. No assumptions are made about how they handle existing values. Most of the time they just overwrite them.
...
However, a set of addtionnal functions is available to help writing the rules.
